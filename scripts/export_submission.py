...
def create_exclude_config_list(category_dir):
    # look for all directories under closed/<SUBMITTER>/configs, closed/<SUBMITTER>/results, and 
    # create a black list of configs to remove for submission code trimming.
    white_list = set()   
    config_directories = set()
    # Get all submitted benchmarks's directory name
    for system_dir in os.scandir(os.path.join(category_dir, SUBMITTER, "results")):
        for benchmark_dir in os.scandir(system_dir.path):
            if benchmark_dir.is_dir():
                white_list.add(f"{benchmark_dir.name}".replace('-99.9', '').replace('-99', ''))

    for path in os.scandir(os.path.join(category_dir, SUBMITTER, "configs")):
        # Get all config directory names
        if path.is_dir():
            config_directories.add(path.name)

    return config_directories - white_list
...
def main(args):
    ...
    for subdir in subdirs:
        ...
        # assume directory is SUBMITTER name
        black_list_config_dir = create_exclude_config_list(subdir)
        exclude_patterns["Not Submitted Benchmarks Configs"] = rf"(.*)/{SUBMITTER}/configs/({'|'.join(black_list_config_dir)})"
