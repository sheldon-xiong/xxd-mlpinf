...
#SBATCH --time=4:00:00
#SBATCH --job-name="gtc_inference-mlperf_inference.llama2-70b-trtllm_executor"
...
$SRUN_HEADER --container-name=$CONTAINER_NAME-generate_engines --nodes=1 --ntasks-per-node=1 --output=slurm-$SLURM_JOB_ID-generate_engine.txt --export=scenario=$scenario --mpi=pmix /bin/bash -c 'export RUN_ARGS="--benchmarks=llama2-70b --scenarios=$scenario --core_type=trtllm_executor" && make generate_engines'

### Launch MLPerf Inf benchmark
#### Add --engine_dir=<path> to RUN_ARGS to pick engine from non default path

## Accuracy run
$SRUN_HEADER --nodes=1 --container-name=$CONTAINER_NAME-run_harness-accuracy --output=slurm-$SLURM_JOB_ID-run_harness-log-accuracy.txt --ntasks-per-node=1 --export=scenario=$scenario /bin/bash -c 'for var in $(compgen -v | grep '^SLURM_'); do unset "$var"; done && make build_loadgen && make run_harness RUN_ARGS="--benchmarks=llama2-70b --scenarios=$scenario --test_mode=AccuracyOnly --core_type=trtllm_executor"'

## Perf run
$SRUN_HEADER --nodes=1 --container-name=$CONTAINER_NAME-run_harness-perf --output=slurm-$SLURM_JOB_ID-run_harness-log-performance.txt --ntasks-per-node=1 --export=scenario=$scenario /bin/bash -c 'for var in $(compgen -v | grep '^SLURM_'); do unset "$var"; done && make build_loadgen && make run_harness RUN_ARGS="--benchmarks=llama2-70b --scenarios=$scenario --test_mode=PerformanceOnly --core_type=trtllm_executor"'
