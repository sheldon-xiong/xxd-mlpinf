...
class AsyncLLMHttpRequestManager:
    ...
    def _init_threading(self):
        ...
        self._request_provider = AsyncHttpLLMClient(self.config, self.max_concurrency)
    ...
    def _init_multiprocess(self):
        ...
        instance_id = str(uuid.uuid4())[:8]  # Use short UUID for cleaner filenames
        ...
        # i found 10k to be too small, been using 50k since
        # might need to increase this in case of different
        ...
        self.request_socket.setsockopt(zmq.SNDHWM, 50000)
        ...
        self.response_socket.setsockopt(zmq.RCVHWM, 50000)
    ...
    def shutdown(self):
        ...
        if self.mode == HarnessWorkerMode.MULTIPROCESS:
            # Send shutdown signal to all workers - daemon processes will die automatically
            # but we still send shutdown signals for graceful cleanup
            try:
                for _ in range(self.workers_per_core):
                    self.request_socket.send(b'__SHUTDOWN__')
            except Exception as e:
                logging.warning(f"Failed to send shutdown signal: {e}")

            # Since processes are daemon, they will die with main process
        ...
        while True:
            try:
                ...
                task = asyncio.create_task(
                    request_provider.process_request_zmq(request, response_socket)
                )
