...
def run_ResNet50_accuracy(engine_file, batch_size, num_images, verbose=False):
    ...
    runner = EngineRunner.from_file(engine_file, verbose=verbose)
    input_tensor_name = runner.engine.get_tensor_name(0)
    input_dtype = runner.engine.get_tensor_dtype(input_tensor_name)
    input_format = runner.engine.get_tensor_format(input_tensor_name)
